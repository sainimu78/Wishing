...
namespace NiflectGen
{
	...
	CInheritableTypeRegCodeWriter2::CInheritableTypeRegCodeWriter2(const CBindingAccessorIndexedNode& bindingTypeIndexedRoot, const Niflect::TArrayNif<CBindingAccessorIndexedNode>& vecMemberIndexedRoot, const CTaggedType* baseTaggedType)
...
;
			...
			for (auto& it : m_vecMemberIndexedRoot)
				linesMembers.push_back(it.m_signature);
...
R"(static void StaticInitType()
{
	...
	type->InitInheritableTypeMeta(StaticGetType<${Bucuo}>);
		...
		if (m_baseTaggedType != NULL)
		{
			...
			CLabelToCodeMapping map;
			MapLabelToText(map, "Nihao", m_bindingTypeIndexedRoot.m_signature);
			MapLabelToText(map, "Bucuo", m_baseTaggedType->m_classDeclIndexedRoot.m_signature);
			Niflect::TSet<Niflect::CString> setReplacedLabel;
			tpl1.ReplaceLabels(map, lines, &setReplacedLabel);
		}
	}
